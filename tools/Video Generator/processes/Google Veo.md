...
python3 scripts/generate_video_google.py "your prompt here"
```

**With aspect ratio:**
```bash
python3 scripts/generate_video_google.py "your prompt" --aspect-ratio 16:9
```

**With custom output:**
```bash
python3 scripts/generate_video_google.py "your prompt" --output-dir /path/to/folder --output filename.mp4
```
...
```bash
python3 scripts/generate_video_google.py "subtle movement, gentle breathing" --image input.jpg
```
...
```bash
python3 scripts/generate_video_google.py "gentle swaying motion" --image input.jpg --loop
```
...
```bash
python3 scripts/generate_video_google.py "subtle hand movements" --image input.jpg --extensions 2
```
...
```bash
python3 scripts/generate_video_google.py "continue the motion" --video initial.mp4 --extensions 1
```
...
```bash
python3 scripts/generate_video_google.py \
...
python3 scripts/generate_video_google.py "person walking" --image start.jpg

# Extend it further
python3 scripts/generate_video_google.py "continue walking smoothly" \
...
**Concatenation error:** Ensure all dependencies from `requirements.txt` are installed (MoviePy is required for joining segments).
