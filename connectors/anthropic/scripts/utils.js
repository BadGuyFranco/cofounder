...
const memoryEnvPath = join(__dirname, '..', '..', '..', '..', 'memory', 'connectors', 'anthropic', '.env');
...
export function loadConfig() {
  if (existsSync(memoryEnvPath)) {
    ...
    console.error('Create /memory/connectors/anthropic/.env with ANTHROPIC_API_KEY=...');
...
export async function apiRequest(path, options = {}, config = null) {
  ...
  const response = await fetch(`${ANTHROPIC_BASE_URL}${path}`, fetchOptions);
